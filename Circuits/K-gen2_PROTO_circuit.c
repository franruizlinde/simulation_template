...
export function conf = customConfig() {
    ...
    // * * * Insert your missions below : * * * * *
   	...
   	conf->missions.{end+1} = varspc(
    		...
    		'missAuto',1,
    		'objectives', {
    			   varspc('task', 'pick', 'dst', 'conv_south'),
    			   varspc('task', 'move', 'dst', 'move_1','destinationTime',15),
             varspc('task', 'drop', 'grp', 'rack_west'),
    		},
    		'flows', {
    ...
    conf->missions.{end+1} = varspc(
        ...
        'missAuto',1,
        'objectives', {
             varspc('task', 'pick', 'grp', 'rack_west'),
             varspc('task', 'drop', 'grp', 'rack_east'),
        },
        'flows', {
